...
public class ClusteredBridgeReconnectTest extends ClusterTestBase {
   ...
   public void testPauseAddressBlockingSnFQueue() throws Exception {
      ...
      for (int i = 0; i < num; i++) {
         ...
         Assert.assertEquals("from producer 0", propValue);
         m.acknowledge();
      }
      ...
      Bindings bindings0 = servers[0].getPostOffice().getBindingsForAddress(new SimpleString("queues.testaddress"));
      Assert.assertNotNull(bindings0);
      Assert.assertEquals(2, bindings0.getBindings().size());
      ...
      Assert.assertTrue(localBindingPaused);
      Assert.assertFalse(remoteBindingPaused);
