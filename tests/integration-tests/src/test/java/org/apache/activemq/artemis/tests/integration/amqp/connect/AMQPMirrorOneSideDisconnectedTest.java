...
public class AMQPMirrorOneSideDisconnectedTest extends ActiveMQTestBase {
   ...
   public void testOneSideDisconnected() throws Exception {
      ...
      serverA.start();
      serverB.start();
      serverA.createQueue(new QueueConfiguration(queueName).setRoutingType(RoutingType.ANYCAST));
      serverA.createQueue(new QueueConfiguration(divertedQueue).setRoutingType(RoutingType.ANYCAST));
      try {
         serverB.createQueue(new QueueConfiguration(queueName).setRoutingType(RoutingType.ANYCAST));
         serverB.createQueue(new QueueConfiguration(divertedQueue).setRoutingType(RoutingType.ANYCAST));
