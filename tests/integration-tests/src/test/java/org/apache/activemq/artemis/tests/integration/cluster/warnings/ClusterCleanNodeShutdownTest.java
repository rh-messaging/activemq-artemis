...
public class ClusterCleanNodeShutdownTest extends ClusterTestBase {
   ...
   private static final Logger logger = Logger.getLogger(ClusterCleanNodeShutdownTest.class);
   ...
   public void testNoWarningErrorsDuringRestartingNodesInCluster() throws Exception {
      ...
      try {
         ...
         logger.debug("*****************************************************************************");
         Assert.assertFalse("Connection failure detected for an expected DISCONNECT event", AssertionLoggerHandler.findText("AMQ212037", " [code=DISCONNECTED]"));
         Assert.assertFalse("WARN found", AssertionLoggerHandler.hasLevel(Level.WARNING));
      } finally {
