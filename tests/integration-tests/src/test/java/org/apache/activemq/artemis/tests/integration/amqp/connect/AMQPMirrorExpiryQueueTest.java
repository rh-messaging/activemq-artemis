...
public class AMQPMirrorExpiryQueueTest extends ActiveMQTestBase {
   ...
   protected ActiveMQServer createServer(int port, String brokerName) throws Exception {
      ...
      server.getConfiguration().addAddressSetting("#", new AddressSettings().setExpiryAddress(SimpleString.toSimpleString(EXPIRY_QUEUE)));
      return server;
   }
   ...
   public void testExpiryOnMirrorSNF() throws Exception {
      ...
      ActiveMQServer serverA = createServer(5671, getName() + "_A");

      {
         AMQPBrokerConnectConfiguration amqpConnection = new AMQPBrokerConnectConfiguration(getName() + "_willNeverConnect", "tcp://localhost:6671").setReconnectAttempts(1).setRetryInterval(10);
      ...
      String queueName = getName() + "_" + RandomUtil.randomString();

      serverA.setIdentity(getName() + "_A");
      serverA.start();

      serverA.createQueue(new QueueConfiguration(queueName).setName(queueName).setRoutingType(RoutingType.ANYCAST));
      Queue expiryA = serverA.createQueue(new QueueConfiguration(EXPIRY_QUEUE).setName(EXPIRY_QUEUE).setRoutingType(RoutingType.ANYCAST));
      Queue snfQueue = serverA.locateQueue(QueueImpl.MIRROR_ADDRESS + "_" + getName() + "_willNeverConnect");
