...
public abstract class RestMessageContext implements Closeable {
   ...
   private void prepareSelf() throws IOException {
      ...
      HttpGet get = connection.prepareGet(destLink);
      HttpResponse response = connection.request(get);
   ...
   public int postMessage(String content, String type) throws IOException {
      ...
      HttpPost post = connection.preparePost(postUri);
      HttpResponse response = connection.post(post, type, content);
      int code = -1;
      try {
   ...
   public boolean acknowledgement(boolean ackValue) throws IOException {
      ...
      if (ackUri != null) {
         HttpPost post = connection.preparePost(ackUri);
         HttpResponse response = connection.post(post, "application/x-www-form-urlencoded",
   ...
   public String pullMessage() throws IOException {
      ...
      HttpPost post = connection.preparePost(msgPullUri);
      HttpResponse response = connection.post(post);
      int code = ResponseUtil.getHttpCode(response);
      try {
         if (code == 200) {
            // success
   ...
   public void setUpPush(String pushTarget) throws Exception {
      ...
      HttpPost post = connection.preparePost(pushLink);
      HttpResponse response = connection.post(post, "application/xml", pushRegXml);
      int code = ResponseUtil.getHttpCode(response);
      try {
         if (code != 201) {
