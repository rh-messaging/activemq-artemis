...
public class JmsReplyToTempQueueTest extends VersionedBase {
   ...
   public void testJmsReplyToTempQueue() throws Throwable {
      evaluate(serverClassloader, "jmsReplyToTempQueue/artemisServer.groovy", serverFolder.getAbsolutePath(), server);
      ...
      try {
         assertTrue(senderLatch.await(10, TimeUnit.SECONDS), "Sender did not get message from temporary queue");
