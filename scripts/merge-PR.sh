...
git checkout $REDHAT_DOWNSTREAM/2.9.0.jbossorg-x -B 2.9.0.jbossorg-x
git checkout $REDHAT_DOWNSTREAM/pr/$1 -B $1
git pull --rebase $REDHAT_DOWNSTREAM 2.9.0.jbossorg-x
git checkout 2.9.0.jbossorg-x
...
echo "git push downstream 2.9.0.jbossorg-x"
