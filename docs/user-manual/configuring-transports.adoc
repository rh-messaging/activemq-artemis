...
It specifies the path to a Certificate Revocation List (CRL) file for additional certificate validation when using PKIX trust manager.
The CRL file contains a list of certificates that have been revoked and should no longer be trusted.
Default is `null`.

crcOptions::
This is valid on either an `acceptor` or `connector`.
It specifies a comma-separated list of PKIXRevocationChecker options to configure certificate revocation checking behavior when using PKIX trust manager.
...
ocspResponderURL::
This is valid on either an `acceptor` or `connector`.
It specifies the URL of the OCSP (Online Certificate Status Protocol) responder to use for certificate revocation checking when using PKIX trust manager.
