...
While the canonical git repository is hosted on Apache hardware at https://gitbox.apache.org/repos/asf/artemis.git contributors are encouraged (but not required) to use a mirror on GitHub for collaboration and pull-request review functionality.
Follow the steps below to get set up with GitHub, etc.

If you do not wish to use GitHub for whatever reason you can follow the overall process outlined in the "Typical development cycle" section below but instead attach https://git-scm.com/docs/git-format-patch[a patch file] to the related JIRA or an email to the https://artemis.apache.org/contact[dev list].
...
[,sh]
----
$ git clone git@github.com:<your-user-name>/artemis.git
...
. https://artemis.apache.org/contact[Discuss your planned changes] (if you want feedback)
...
To git@github.com:<your-user-name>/artemis.git
